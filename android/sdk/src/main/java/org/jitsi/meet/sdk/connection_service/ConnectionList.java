...
/**
 * This holds all connections created by the {@link ConnectionService}. It serves as a bridge
 * for storing the state used by both {@link RNConnectionService} and {@link ConnectionService}.
 * Pawel: ideally this state would live inside the {@link ConnectionService}, but I haven't found
 * a clean way to access the {@link ConnectionService} from {@link RNConnectionService}.
 *
 * @author Pawel Domas
 */
...
public class ConnectionList
{
    ...
    /**
     * Used to adjusts the connection's state to {@link android.telecom.Connection#STATE_ACTIVE}.
     *
    ...
    void setConnectionDisconnected(String callUUID, DisconnectCause cause) {
        ...
        if (connection != null) {
            // Note that the connection is not removed from the list here, but in ConnectionImpl's
            // state changed callback. It's a safer approach, because in case the app would crash
            // on the JavaScript side the calls would be cleaned up by the system they would still
    ...
    /**
     * Unregisters a start call promise. Must be called after the Promise is rejected or resolved.
     *
     * @param uuid the call UUID which identifies the call to which the promise belongs to.
     * @return the unregistered Promise instance or <tt>null</tt> if there wasn't any for the given
     * call UUID.
     */
